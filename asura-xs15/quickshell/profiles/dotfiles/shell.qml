...
ShellRoot {
    ...
    Process {
        command: ["bash", Quickshell.shellDir + "/count_tiled.sh"]
        running: true
        stdout: SplitParser {
            onRead: data => {
    ...
    function hyprlandAvailable() {
        return Hyprland.focusedWorkspace !== null || Hyprland.workspaces.values.length > 0;
    }
    ...
    Process {
        ...
        command: ["sh", "-c", "echo false"]
        running: true
        stdout: SplitParser { onRead: data => { root.batteryMode = (data.trim() === 'true'); } }
    }
    Process {
        id: pToggleBatteryMode
        command: ["notify-send", "-a", "asura-dotfiles-shell", "Battery mode toggle is disabled in this Hyprland test profile"]
    }
    ...
    Process { id: pGpu; command: ["notify-send", "-a", "asura-dotfiles-shell", "GPU switching is disabled in this Hyprland test profile"] }

    Process { id: pGpuInt; command: ["notify-send", "-a", "asura-dotfiles-shell", "GPU switching is disabled in this Hyprland test profile"] }
    Process { id: pGpuHyb; command: ["notify-send", "-a", "asura-dotfiles-shell", "GPU switching is disabled in this Hyprland test profile"] }

    Process { id: pNoteHyprland; command: ["xdg-open", "/etc/nixos/asura-xs15/hyprland"] }
    Process { id: pNoteWaybar; command: ["xdg-open", "/etc/nixos/docs/QUICKSHELL_PROFILES.md"] }
    Process { id: pNoteTofi; command: ["xdg-open", "/etc/nixos/asura-xs15/quickshell"] }
    Process { id: pNoteKitty; command: ["xdg-open", "/etc/nixos/home/programs/terminal"] }
    Process { id: pNoteFoot; command: ["xdg-open", "/etc/nixos/home/programs/terminal"] }
    Process { id: pNoteGhostty; command: ["xdg-open", "/etc/nixos/home/programs/terminal"] }
    Process { id: pNoteFish; command: ["xdg-open", "/etc/nixos/home/programs/terminal"] }
    Process { id: pNoteFastfetch; command: ["xdg-open", "/etc/nixos/home/programs/fastfetch"] }
    Process { id: pNoteQuickshell; command: ["xdg-open", Quickshell.shellDir] }
    ...
    Rectangle {
        ...
        RowLayout {
            ...
            Repeater {
                ...
                Mod {
                    property bool hasHyprland: root.hyprlandAvailable()
                    property var ws: hasHyprland ? Hyprland.workspaces.values.find(w => w.id === modelData) : undefined
                    property bool isActive: hasHyprland && Hyprland.focusedWorkspace != null && Hyprland.focusedWorkspace.id === modelData

                    text: modelData
                    textColor: isActive ? root.colFg : root.colMuted
                    bgColor: "transparent"
                    show: (hasHyprland ? (ws !== undefined || isActive) : true) && !root.showOsd
