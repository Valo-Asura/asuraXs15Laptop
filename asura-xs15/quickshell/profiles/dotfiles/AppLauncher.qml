...
PanelWindow {
    ...
    Process {
        id: pGetApps
        command: ["python3", Quickshell.shellDir + "/get_apps.py"]
        stdout: SplitParser {
                ...
                TextField {
                    ...
                    Keys.onReturnPressed: {
                        if (listView.currentIndex >= 0 && listView.currentIndex < appModel.count) {
                            ...
                            pExec.command = app.desktop ? ["gio", "launch", app.desktop] : ["sh", "-lc", "exec " + app.exec];
                            pExec.running = true;
                            show = false;
                        }
                    }
                    ...
                    delegate: Rectangle {
                        ...
                        MouseArea {
                            ...
                            onClicked: {
                                ...
                                pExec.command = model.desktop ? ["gio", "launch", model.desktop] : ["sh", "-lc", "exec " + model.exec];
                                pExec.running = true;
