...
PanelWindow {
    ...
    Process {
        id: pGetThemes
        command: ["sh", "-c", "printf '%s\\n' default noctalia asura-hyprland"]
        stdout: SplitParser {
            onRead: data => {
                var d = data.trim();
                if (d.length > 0) {
                ...
                TextField {
                    ...
                    Keys.onReturnPressed: {
                        if (listView.currentIndex >= 0 && listView.currentIndex < themeModel.count) {
                            ...
                            pExec.command = ["notify-send", "-a", "asura-dotfiles-shell", "Theme switch disabled in this Hyprland test profile", t];
                            pExec.running = true;
                            show = false;
                        }
                    }
                    ...
                    delegate: Rectangle {
                        ...
                        MouseArea {
                            ...
                            onClicked: {
                                ...
                                pExec.command = ["notify-send", "-a", "asura-dotfiles-shell", "Theme switch disabled in this Hyprland test profile", model.name];
                                pExec.running = true;
