...
CustomMouseArea {
    ...
    function panelHoverSize(panel: Item, fallback: real): real {
        return Math.max(Config.border.minThickness, Config.border.thickness + panel.height, fallback);
    }

    function inTopPanel(panel: Item, x: real, y: real, fallback = 0): bool {
        return y < panelHoverSize(panel, fallback) + panel.y && withinPanelWidth(panel, x, y);
    }

    function inBottomPanel(panel: Item, x: real, y: real, isCorner = false, fallback = 0): bool {
        return y > height - panelHoverSize(panel, fallback) - (isCorner ? Config.border.rounding : 0) && withinPanelWidth(panel, x, y);
    }
    ...
    onPositionChanged: event => {
        ...
        if (Config.launcher.showOnHover) {
            if (!visibilities.launcher && inBottomPanel(panels.launcher, x, y, false, Config.launcher.dragThreshold))
                visibilities.launcher = true;
        } else if (pressed && inBottomPanel(panels.launcher, dragStart.x, dragStart.y, false, Config.launcher.dragThreshold) && withinPanelWidth(panels.launcher, x, y)) {
        ...
        const showDashboard = Config.dashboard.showOnHover && inTopPanel(panels.dashboard, x, y, Config.dashboard.dragThreshold);
        ...
        if (pressed && inTopPanel(panels.dashboard, dragStart.x, dragStart.y, Config.dashboard.dragThreshold) && withinPanelWidth(panels.dashboard, x, y)) {
