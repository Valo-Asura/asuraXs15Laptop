...
Singleton {
    ...
    property bool thermalWarned

    readonly property real thermalWarnTemp: 88
    readonly property real thermalResetTemp: 72
    ...
    function checkThermals(): void {
        const maxTemp = Math.max(cpuTemp || 0, gpuTemp || 0);
        if (maxTemp >= thermalWarnTemp && !thermalWarned) {
            thermalWarned = true;
            Toaster.toast(qsTr("High temperature"), qsTr("CPU/GPU temperature is %1°C. NBFC should ramp both fans automatically.").arg(Math.ceil(maxTemp)), "device_thermostat", Toast.Warning);
        } else if (maxTemp <= thermalResetTemp) {
            thermalWarned = false;
        }
    }
    ...
    Timer {
        ...
        triggeredOnStart: true
        onTriggered: {
            if (!thermalProbe.running)
                thermalProbe.running = true;
        }
    }
    ...
    Process {
        ...
        command: ["sh", "-c", "max=0; for d in /sys/class/hwmon/hwmon*; do name=$(cat $d/name 2>/dev/null || true); case $name in coretemp|k10temp|amdgpu|nvidia*) for f in $d/temp*_input; do v=$(cat $f 2>/dev/null || true); [ -n \"$v\" ] || continue; c=$((v / 1000)); [ \"$c\" -gt \"$max\" ] && max=$c; done;; esac; done; printf '%s\\n' \"$max\""]
        stdout: StdioCollector {
            onStreamFinished: {
